...
function AccountChangePassword({user}) {
    ...
    const options = {
        ...
        .required(),
        confirmPw:yup.string().trim().oneOf([yup.ref('newPw'), null], 'Passwords must match'),
    };
    const {
        register,
        handleSubmit,
        ...
        <form className={cx('change-pw-wrap')} onSubmit={handleSubmit(onSubmit)}>
            ...
            <div className={cx('field-item')}>
              ...
              <div className={cx('user-action-container')}>
                    ...
                    <Link to={ACCOUNT_PATHS.ACCOUNT+ACCOUNT_PATHS.LOST_PASSWORD} className={cx("lost-password")}>Lost your password?</Link>
              </div>
        </form>
    );
}
