    ...
    async function (accessToken, refreshToken, profile, cb) {
      try {
        if (profile?.id) {
          ...
          if (user.length === 0) {
            await authSocialService.insertUserSocial(profile.id,constant.ROLE_DEFAULT,profile.displayName,profile.emails[0].value,profile.photos[0].value,constant.TYPE_LOG_SOCIAL,code)
          } else{
